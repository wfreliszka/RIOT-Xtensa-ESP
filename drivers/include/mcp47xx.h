 ...
 *
 * The driver is also compatible with the DAC extension API so that MCP47xx
 * DAC devices can be used as DAC extensions with the standard peripheral
 * DAC interface.
 *
 * Please refer the test application in `tests/driver_mcp47xx` for an example
 * on how to use the driver either directly or through the DAC extension
 * interface.
 *
 * ## SAUL Capabilities
 *
 ...
 * of type #mcp47xx_params_t. As an example, the default configuration for one MCP4725 device is
 * defined in `drivers/mcp47xx/mcp47xx_params.h`.
 *
 ...
 *                              see #mcp47xx_error_codes_t
 */
int mcp47xx_dac_init(mcp47xx_t *dev, dac_t chn);
 ...
 * @retval  none
 */
void mcp47xx_dac_set(mcp47xx_t *dev, dac_t chn, uint16_t value);
 ...
 * @retval      none
 */
void mcp47xx_dac_poweron(mcp47xx_t *dev, dac_t chn);
 ...
 * @retval      none
 */
void mcp47xx_dac_poweroff(mcp47xx_t *dev, dac_t chn);
