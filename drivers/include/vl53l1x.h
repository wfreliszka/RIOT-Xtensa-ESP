 ...
 * :-------------------|:----------------------|:----------------------------
 * \ref vl53l1x        | Standard driver       | Common functionality and medium size
 ...
 *
 * \anchor vl53l1x_st_api
 * ### ST VL53L1X API driver
 *
 * This variant of the driver uses ST STSW-IMG007 VL53L1X API as package.
 ...
 *
 * Even though the sensor is calibrated by ST during the final modul test and
 ...
 *
 * @note Since the ST STSW-IMG007 VL53L1X API package is quite complex and very
 * large, this driver variant should only be used when memory requirements
 * are not an issue.
 *
 * \anchor vl53l1x
 * ### <a name="vl53l1x"> Standard driver vl53l1x </a>
 *
 ...
 *
 * \anchor vl53l1x_basic
 * ### <a name="vl53l1x_basic"> Basic driver vl53l1x_basic </a>
 *
 ...
 *
 * @note Distance mode CANNOT be configured when module \ref vl53l1x_basic is
 * enabled.
 */
typedef enum {
    #if MODULE_VL53L1X_ST_API
...
#if MODULE_VL53L1X_ST_API || DOXYGEN
/**
 ...
 * conditions under which an theshold interrupt will be triggered.
 *
 * @note Threshold interrupts are ONLY available when module
 * \ref vl53l1x_st_api is enabled. Otherwise only data-ready interrupts
 * can be used.
 */
typedef enum {
 ...
 * either on new data in each measurement cycly or when the treshold condition
...
#endif  /* MODULE_VL53L1X_ST_API || DOXYGEN */
...
typedef struct {
    ...
    #elif !MODULE_VL53L1X_BASIC
    bool     calibrated;         /**< Device is already calibrated */
    uint16_t fast_osc_frequency; /**< Fast OSC Frequency in fixe point 4.12 format */
 ...
 */
int vl53l1x_data_ready(const vl53l1x_t *dev);
 ...
 *
 * @note Since reading any data resets the interrupt as well as the data-ready
 ...
 * @retval  VL53L1X_ERROR_*    negative error code
 */
int vl53l1x_read_details(const vl53l1x_t *dev,
 ...
 *
 * @note  This function is NOT available when module \ref vl53l1x_basic is used.
 *
 * @param[in]   dev     device descriptor of VL53L1X sensor
 *
 * @retval  VL53L1X_OK         on success
 ...
 *
 * @note
 * - The inter-measurement period MUST be longer than the timing budget
...
/**
 * @brief   Set the region of interrest (ROI)
 *
 ...
 * @retval  VL53L1X_ERROR_*    negative error code
 */
int vl53l1x_set_roi(const vl53l1x_t *dev, vl53l1x_roi_t *roi);

/**
 * @brief   Get the region of interrest (ROI)
 *
 * @note  This function is NOT available when module \ref vl53l1x_basic is used.
 *
 * @param[in]   dev     device descriptor of VL53L1X sensor
 ...
 * @retval  VL53L1X_ERROR_*    negative error code
 */
int vl53l1x_get_roi(const vl53l1x_t *dev, vl53l1x_roi_t *roi);
 ...
 * - The interrupt configuration for threshold interrupts is ONLY used when
 * module \ref vl53l1x_st_api is enabled. Otherwise, only data-ready interrupts
 * are used independent on the configuration given by parameter \p cfg.
 * In later case, parameter \p cfg can be NULL.
 *
 * @param[in]   dev     device descriptor of VL53L1X sensor
